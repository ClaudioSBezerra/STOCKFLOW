...
- E-mail único garantido atomicamente (409 em duplicata) — **revisado em 2026-09-24:** único entre todas as Empresas reais; o Ambiente de Treinamento pode repetir o e-mail de uma conta da sua Empresa real (ver FR-42).
...
- **Unicidade de e-mail (FR-3) entre as Empresas reais** *(revisado em 2026-09-24 — antes era por Empresa, o que permitia o mesmo e-mail em Empresas diferentes)*: um e-mail tem no máximo uma conta somando todas as Empresas reais. Convite, autocadastro e o cadastro do primeiro `adm` (FR-41) recusam um e-mail que já tenha conta em outra Empresa real. No Ambiente de Treinamento (FR-43) o e-mail pode repetir o de uma conta da **sua** Empresa real — é a mesma pessoa praticando —, nunca o de uma conta de outra Empresa real.
- Login (FR-1, FR-34) resolve a Empresa a partir do contexto de acesso (FR-40) ou, na raiz do domínio, a partir da conta (FR-54) — nunca de um seletor manual de Empresas na tela de login. A única pergunta possível é "Ambiente real ou Treinamento?", e só depois de a senha ser conferida (FR-54).
...
#### FR-54: Login na raiz do domínio, sem a Empresa na URL *(novo — 2026-09-24)*
Quem abre o endereço do stockflow sem `/e/{empresa}` entra só com e-mail e senha; o sistema descobre a Empresa pela conta e leva a pessoa para dentro dela.
**Consequences:**
- **Uma conta com aquele e-mail e senha:** entra direto na Empresa da conta; dali em diante o endereço mostra `/e/{empresa}`, como hoje.
- **Conta na Empresa real e no Ambiente de Treinamento com o mesmo e-mail:** depois de a senha ser conferida, o login pergunta "Ambiente real ou Treinamento?" (com o nome de cada Empresa). Se a senha só confere numa das duas, entra direto nela, sem perguntar.
- **Não revela quem tem conta:** e-mail inexistente e senha errada recebem a mesma resposta e demoram o mesmo; a pergunta real/Treinamento só aparece depois da senha conferida. Falhas contam no mesmo bloqueio por tentativas de FR-36, em cada conta daquele e-mail.
- **Mesmas regras do login de hoje:** conta desativada, e-mail não confirmado, Empresa desativada (FR-41) e o segundo fator (FR-37, FR-53) valem exatamente como no login pelo endereço da Empresa — a raiz só descobre a Empresa, não abre exceção.
- **Esqueci a senha na raiz:** também funciona só com o e-mail; o link de redefinição (FR-32) chega com o endereço da Empresa certa (um e-mail por conta, se houver real e Treinamento). A resposta é sempre a mesma, exista ou não a conta.
- **Domínio de um único cliente:** um servidor dedicado a uma só Empresa (ex. `suprimentos.fcxlabs.com`) pode ser configurado para que a raiz abra direto o login daquela Empresa. Sem essa configuração, vale o login pela conta descrito acima.
- **Endereços com a Empresa continuam funcionando:** links já distribuídos (`/e/{empresa}`), convites (FR-42) e o login SSO (FR-34, que continua disponível só dentro do endereço da Empresa) não mudam.
**Out of Scope:** trocar de Empresa sem sair (cada conta pertence a uma só Empresa real); SSO na raiz do domínio; subdomínio por Empresa.
...
- **Unicidade** *(revisado em 2026-09-24)*: papel `adm` (FR-40) é único por Empresa; e-mail de Usuário (FR-3, FR-42) é único entre as Empresas reais, podendo repetir apenas entre uma Empresa real e o seu Ambiente de Treinamento — garantido no banco, não só na tela.
