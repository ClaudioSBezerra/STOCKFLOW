...
## I. Multi-Empresa V1 (2026-09-10) — pesquisa de mercado e decisões deliberadas

Pesquisa completa em `_bmad-output/planning-artifacts/research/market-multi-tenancy-e-sandbox-de-treinamento-b-2026-09-10/research.md`. Resumo e como cada achado se relaciona com as decisões de FR-40 a FR-44 do PRD:

**Membership de Usuário: 1 Empresa vs. N Empresas.** O padrão dominante em SaaS B2B horizontal (GitHub, Linear, Vercel, confirmado via documentação oficial) é usuário pertencer a **N organizações simultâneas**, com papel independente em cada uma, e um seletor de organização (tipicamente no canto superior esquerdo da UI). O stockflow **diverge deliberadamente disso na V1** (FR-42: 1 Usuário = 1 Empresa) por dois motivos específicos deste domínio: (1) urgência declarada da V1 — um modelo N:N exige mais superfície de decisão de produto (troca de contexto, papel por organização, UI de seletor) que não é necessária para liberar rápido; (2) diferente de ferramentas horizontais (onde é comum colaborar em múltiplas organizações ao mesmo tempo, ex. um freelancer em vários workspaces), um funcionário de almoxarifado/obra tipicamente trabalha para uma única empresa por vez — não há caso de uso identificado hoje que exija pertencer a duas Empresas simultaneamente. Fica como candidato explícito de revisão na V2, junto com "negócios entre Empresas" (compartilhar Catálogo/Estoque/Pedidos), que é exatamente o cenário que tornaria N:N necessário.

**Papel "Dono da Plataforma" (super-admin cross-tenant).** Confirmado como padrão real e recomendado (Atlassian Cloud: org admin/site admin em camadas; Microsoft Power Platform: service admin role; Frontegg recomenda restringir a poucas pessoas). Nesses produtos de referência, esse papel tipicamente TEM acesso a mecanismos cross-tenant (impersonation para suporte, billing, analytics agregada). O stockflow adota uma versão **mais restrita**: FR-41 explicitamente NEGA ao Dono da Plataforma qualquer acesso a conteúdo operacional (Catálogo/Estoques/Pedidos) de qualquer Empresa — só metadado (nome, status, admin responsável). Decisão deliberada, consistente com "isolamento total" ser o princípio inegociável desta V1; revisitar apenas se surgir necessidade real de suporte cross-Empresa (ex. depuração de um bug reportado por um cliente), tratada então como exceção auditada, não como acesso padrão.

**Ambiente de Treinamento automático vs. sob demanda/por plano.** O padrão dominante em produtos maduros (Salesforce com 4 tipos de sandbox, NetSuite, HubSpot) é o sandbox ser **gated por plano/tier** ou criado sob demanda — não provisionado automaticamente para todo cliente. O stockflow diverge disso deliberadamente: FR-43 provisiona o Ambiente de Treinamento **automaticamente para toda Empresa**, sem gate de plano (coerente com não haver planos comerciais nesta V1 — ver Non-Goals). Isso é tratado como um diferencial de produto para o onboarding (todo cliente novo já treina o time sem esperar nem pedir), não uma omissão da pesquisa — é a decisão explícita do usuário nesta rodada de PRD, mantida mesmo divergindo do padrão de mercado.

**Distinção visual do Ambiente de Treinamento.** Os produtos de referência usam majoritariamente subdomínio/URL dedicado (ex. Salesforce injeta "sandbox" no hostname) — banners internos sozinhos são vistos como insuficientes. Não é FR neste PRD (é decisão de UX/Arquitetura), mas fica registrado aqui como input direto para a fase de UX: considerar uma URL ou indicador visual persistente e difícil de ignorar para o Ambiente de Treinamento, para reduzir risco de um usuário confundir treinamento com produção.

**Lacuna reconhecida:** a pesquisa não encontrou exemplos primários de WMS/ERP multi-empresa (NetSuite OneWorld, Odoo, SAP Business One) — o análogo mais próximo do domínio de estoque/obra do stockflow. Se a Arquitetura precisar de mais rigor no desenho de isolamento (schema-per-tenant vs. row-level vs. database-per-tenant), vale um aprofundamento pontual nesses produtos especificamente antes de fechar a decisão técnica.
