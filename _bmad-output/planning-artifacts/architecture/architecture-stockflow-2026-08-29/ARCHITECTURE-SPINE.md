...
- **Rule:**
  - **Dado:** `usuarios.empresa_raiz_id UUID NULL REFERENCES empresas(id)` = a Empresa real da conta (`COALESCE(empresas.empresa_origem_id, empresas.id)` da Empresa da conta), com `CHECK ((empresa_id IS NULL) = (empresa_raiz_id IS NULL))`. Preenchida por trigger `BEFORE INSERT OR UPDATE OF empresa_id`, o que cobre todo ponto de `INSERT INTO usuarios` (cadastro, convite, primeiro `adm`, CLIs de seed e migração) e o backfill da Multi-Empresa, sem cada um precisar lembrar. Backfill na mesma migration. **Revisado em 2026-09-24:** a primeira versão era `NOT NULL` e abortava num banco novo por causa da conta sintética sem Empresa da 000022 (derrubou stockflow.fbtechia.com); conta sem Empresa fica fora da regra de e-mail (NULL nunca conflita), e `runMigrations` sai sozinho do estado "49 dirty" quando nada da 000049 foi aplicado.
