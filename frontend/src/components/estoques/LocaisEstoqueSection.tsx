...
/**
 * Seção "Locais" da página `/estoques` (Story 2.1, spec-2-1). Um `Card` com
 * (a) um formulário `<Input>` + `<Button>` "Adicionar estoque" que faz
 * `POST /api/estoques` com `{ nome }`, e (b) a lista de Estoques de
 * `GET /api/estoques`, carregada no `useEffect` de mount.
 *
        ...
        {!erroCarregar && estoques.length > 0 && (
          <ul className="flex flex-col gap-2">
            {estoques.map((e) => (
              <li
                key={e.id}
                className="text-body border-b border-border pb-2 last:border-b-0 last:pb-0"
              >
                {e.nome}
              </li>
            ))}
